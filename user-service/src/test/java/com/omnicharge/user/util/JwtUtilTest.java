...
class JwtUtilTest {
    ...
    void refreshToken_Flow() {
        String token = jwtUtil.generateRefreshToken(1L, "9876543210");
        assertNotNull(token);
    ...
    void validateRefreshToken_WrongType() {
        String token = jwtUtil.generateAccessToken(1L, "M", "R", true);
        assertThrows(io.jsonwebtoken.JwtException.class, () -> jwtUtil.validateRefreshToken(token));
    }

    @Test
    void testGetters() {
        assertEquals(1800000L, jwtUtil.getAccessTokenExpiration());
        assertEquals(604800000L, jwtUtil.getRefreshTokenExpiration());
    }

    @Test
    void init_WithNullSecret() {
