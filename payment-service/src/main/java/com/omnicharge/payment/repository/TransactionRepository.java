...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.userId = :userId " +
            "AND (:#{#minAmount == null} = true OR t.amount >= :minAmount) " +
            "AND (:#{#maxAmount == null} = true OR t.amount <= :maxAmount) " +
            "AND (:#{#status == null} = true OR t.status = :status) " +
            "AND (:#{#startDate == null} = true OR t.createdDate >= :startDate) " +
            "AND (:#{#endDate == null} = true OR t.createdDate <= :endDate)")
    ...
    @Query("SELECT t FROM Transaction t WHERE " +
            "(:#{#userId == null} = true OR t.userId = :userId) " +
            "AND (:#{#minAmount == null} = true OR t.amount >= :minAmount) " +
            "AND (:#{#maxAmount == null} = true OR t.amount <= :maxAmount) " +
            "AND (:#{#status == null} = true OR t.status = :status) " +
            "AND (:#{#startDate == null} = true OR t.createdDate >= :startDate) " +
            "AND (:#{#endDate == null} = true OR t.createdDate <= :endDate) " +
            "AND (:#{#rechargeId == null} = true OR t.rechargeId = :rechargeId)")
