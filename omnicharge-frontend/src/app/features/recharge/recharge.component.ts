            ...
            <form [formGroup]="numberForm">
              ...
              >
                @if (detecting) {
                  ...
                  <ng-container> <mat-icon>search</mat-icon> Detect Operator </ng-container>
                }
              </button>
            </form>
            ...
            @if (!paymentSuccess) {
              ...
              @if (sagaStep) {
                <div class="saga-tracker slide-up">
                  <div
                    class="saga-step"
                    [class.active]="sagaStep === 'INITIATE'"
                    [class.done]="sagaStepDone('INITIATE')"
                  >
                    <mat-icon>{{
                      sagaStepDone('INITIATE') ? 'check_circle' : 'hourglass_empty'
                    }}</mat-icon>
                  ...
                  <div
                    class="saga-step"
                    [class.active]="sagaStep === 'ORDER'"
                    [class.done]="sagaStepDone('ORDER')"
                  >
                    <mat-icon>{{
                      sagaStepDone('ORDER') ? 'check_circle' : 'hourglass_empty'
                    }}</mat-icon>
                  ...
                  <div
                    class="saga-step"
                    [class.active]="sagaStep === 'CHECKOUT'"
                    [class.done]="sagaStepDone('CHECKOUT')"
                  >
                    <mat-icon>{{
                      sagaStepDone('CHECKOUT') ? 'check_circle' : 'hourglass_empty'
                    }}</mat-icon>
                  ...
                  <div
                    class="saga-step"
                    [class.active]="sagaStep === 'VERIFY'"
                    [class.done]="sagaStepDone('VERIFY')"
                  >
                    <mat-icon>{{
                      sagaStepDone('VERIFY') ? 'check_circle' : 'hourglass_empty'
                    }}</mat-icon>
                  ...
                  <div
                    class="saga-step"
                    [class.active]="sagaStep === 'COMPLETE'"
                    [class.done]="sagaStepDone('COMPLETE')"
                  >
                    <mat-icon>{{
                      sagaStepDone('COMPLETE') ? 'check_circle' : 'hourglass_empty'
                    }}</mat-icon>
                  ...
                  <div
                    class="saga-step"
                    [class.active]="sagaStep === 'NOTIFY'"
                    [class.done]="sagaStepDone('NOTIFY')"
                  >
                    <mat-icon>{{
                      sagaStepDone('NOTIFY') ? 'check_circle' : 'hourglass_empty'
                    }}</mat-icon>
              ...
              @if (selectedPlan && detectedOperator && !sagaStep) {
                <div class="summary-card glass">
                  ...
                  <div class="summary-row">
                    <span>Plan</span>
                    <strong
                      >{{ selectedPlan.name }} • {{ selectedPlan.data }} •
                      {{ selectedPlan.validity }} Days</strong
                    >
                  </div>
                ...
                <div class="success-notifications">
                  ...
                  <div class="success-notif">
                    ...
                    <div>
                      ...
                      <span
                        >{{ detectedOperator?.name }} {{ selectedPlan?.data }} activated for
                        {{ numberForm.value.mobileNumber }}</span
                      >
                    </div>
                  </div>
                  <div class="success-notif">
...
export class RechargeComponent implements OnInit, OnDestroy, HasUnsavedChanges {
    ...
    this.operatorService
      ...
      .subscribe({
        ...
        error: (err) => {
          this.detecting = false;
          this.cdr.markForCheck();
          this.snackBar.open(err.error?.message || 'Detection failed', 'Close', {
            duration: 4000,
          ...
          this.paymentService.createOrder(paymentPayload).subscribe({
            next: (paymentRes) => {
              ...
              if (paymentRes.status === 'FAILED' || !paymentRes.razorpayOrderId) {
                ...
                this.snackBar.open('Payment order creation failed. Please try again.', 'Close', {
                  duration: 5000,
                  panelClass: ['error-snackbar'],
                });
                  ...
                  this.paymentService
                    ...
                    .subscribe({
                      next: (txn) => {
                        ...
                        this.rechargeService.completeRecharge(
                          recharge,
                          txn.transactionId || paymentResult.razorpayPaymentId,
                        );
                        ...
                        this.snackBar.open('Payment & Recharge successful! 🎉', 'Close', {
                          duration: 4000,
  ...
  private sagaCompensate(failedAt: string, recharge: Recharge | null, orderId: string): void {
    ...
    if (this.sagaStepsCompleted.includes('ORDER') && orderId) {
      this.paymentService.recordFailedTransaction(
        recharge?.id || 0,
