...
@Component({
    ...
    <div class="dashboard fade-in">
      ...
      <div class="welcome-banner glass-panel slide-down">
        <div class="welcome-content">
          ...
          <h1><span class="highlight">{{ userName }}</span> 👋</h1>
      ...
      <div class="recent-section slide-up">
        <div class="section-header">
          ...
          <button mat-button color="primary" routerLink="/history" class="view-all-btn">View All</button>
        </div>
  ...
  styles: [`
    ...
    .notification-bar {
      ...
      align-items: center;
      background: linear-gradient(135deg, rgba(45, 212, 191, 0.15) 0%, rgba(15, 118, 110, 0.15) 100%);
    ...
    .hover-lift:hover {
      ...
      box-shadow: 0 15px 35px rgba(0, 0, 0, 0.2), 0 0 20px rgba(99, 102, 241, 0.1);
    }
    ...
    .action-icon-wrapper mat-icon { font-size: 28px; width: 28px; height: 28px; }

    .purple-gradient { background: linear-gradient(135deg, #a855f7, #7e22ce); }
    .teal-gradient { background: linear-gradient(135deg, #2dd4bf, #0f766e); }
    .amber-gradient { background: linear-gradient(135deg, #fbbf24, #d97706); }
    ...
    .operator-icon.jio { color: #e53935; background: rgba(229, 57, 53, 0.1); }
    .operator-icon.airtel { color: #d32f2f; background: rgba(211, 47, 47, 0.1); }
    .operator-icon.vi { color: #fbc02d; background: rgba(251, 192, 45, 0.1); }
    ...
    .status-badge.success { background: rgba(44,182,125,0.15); color: #2cb67d; border: 1px solid rgba(44,182,125,0.3); }
    .status-badge.failed { background: rgba(229,49,112,0.15); color: #e53170; border: 1px solid rgba(229,49,112,0.3); }
    .status-badge.initiated, .status-badge.processing { background: rgba(255,137,6,0.15); color: #ff8906; border: 1px solid rgba(255,137,6,0.3); }
    ...
    @media (max-width: 768px) {
      ...
      .quick-actions { grid-template-columns: 1fr; gap: 16px; margin-bottom: 32px; }
      .recent-grid { grid-template-columns: 1fr; }
      .orb-1 { width: 150px; height: 150px; right: -20px; top: -50px; }
      .orb-2 { width: 100px; height: 100px; right: 80px; bottom: -50px; }
      .notification-bar { padding: 12px 16px; flex-direction: column; align-items: flex-start; gap: 12px; }
      .dismiss-btn { position: absolute; right: 8px; top: 8px; }
    }
  `]
})
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    this.notificationService.getNotifications()
      ...
      .subscribe({
        next: (notifs) => {
          ...
          this.recentNotifications = notifs.filter(n => !n.isRead && (n.category === 'PAYMENT' || n.category === 'RECHARGE')).slice(0, 1);
          this.cdr.markForCheck();
        }
      });
  ...
  dismissNotification(): void {
    if (this.recentNotifications.length > 0) {
      this.notificationService.markAsRead(this.recentNotifications[0].id)
      .pipe(takeUntil(this.destroy$))
      .subscribe();
