      ...
      <mat-card class="auth-card slide-up">
          ...
          <mat-step [stepControl]="otpForm">
            <form [formGroup]="otpForm" (ngSubmit)="onVerifyOtp()">
              ...
              <div class="sent-to-info">
                ...
                <span
                  >OTP sent to <strong>+91 {{ registerForm.value.mobileNumber }}</strong></span
                >
              </div>
        ...
        <div class="auth-footer">
          <p>
            ...
            <a routerLink="/auth/login" [queryParams]="returnUrl ? { returnUrl: returnUrl } : {}"
              >Sign In</a
            >
          </p>
        </div>
  ...
  onVerifyOtp(): void {
    ...
    this.authService.verifyPhoneOtp(request).subscribe({
      next: () => {
        this.loading = false;
        this.cdr.markForCheck();
        this.snackBar.open('Welcome to OmniCharge!', 'Close', {
          duration: 3000,
          panelClass: ['success-snackbar'],
        });
      ...
      error: (err) => {
        this.loading = false;
        this.cdr.markForCheck();
        this.snackBar.open(err.error?.message || 'Verification failed', 'Close', {
          duration: 4000,
          panelClass: ['error-snackbar'],
        });
      },
