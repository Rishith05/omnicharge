    ...
    <div class="auth-container">
      ...
      <mat-card class="auth-card slide-up">
          ...
          <form [formGroup]="phoneForm" (ngSubmit)="onSendOtp()">
            <div class="phone-input-group">
              ...
              <mat-form-field appearance="outline" class="phone-field">
                ...
                @if (
                  phoneForm.get('mobileNumber')?.hasError('required') &&
                  phoneForm.get('mobileNumber')?.touched
                ) {
                  <mat-error>Phone number is required</mat-error>
                }
                @if (
                  phoneForm.get('mobileNumber')?.hasError('pattern') &&
                  phoneForm.get('mobileNumber')?.touched
                ) {
        ...
        @if (step === 'otp') {
          <div class="otp-section">
            <div class="sent-to-info">
              ...
              <span
                >OTP sent to <strong>+91 {{ phoneForm.value.mobileNumber }}</strong></span
              >
        ...
        <div class="auth-footer">
          <p>
            New to OmniCharge?
            <a routerLink="/auth/register" [queryParams]="returnUrl ? { returnUrl: returnUrl } : {}"
              >Create Account</a
            >
          </p>
        </div>
  ...
  onSendOtp(): void {
    ...
    this.authService.sendOtp({ mobileNumber: this.phoneForm.value.mobileNumber }).subscribe({
      next: () => {
        ...
        this.cdr.markForCheck();
        this.snackBar.open('OTP sent to your phone!', 'Close', {
          duration: 3000,
