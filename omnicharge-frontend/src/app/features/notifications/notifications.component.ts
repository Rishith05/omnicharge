...
@Component({
  ...
  imports: [
    ...
    MatIconModule, MatButtonModule, MatBadgeModule, MatSnackBarModule,
    ...
    <div class="notifications-page fade-in">
      <div class="page-header">
        ...
        <div class="page-actions">
          <button mat-stroked-button (click)="refreshData()" matTooltip="Refresh notifications" class="refresh-btn">
            <mat-icon>refresh</mat-icon> Refresh
          </button>
          <button mat-raised-button color="primary" (click)="markAllRead()" *ngIf="allNotifications.length > 0">
  ...
  styles: [`
    ...
    .notification-card:hover { transform: translateX(4px); }
    .notification-card.unread { border-left: 3px solid var(--accent-purple); }
    .notif-icon-wrap {
      width: 44px; height: 44px; border-radius: 12px;
      display: flex; align-items: center; justify-content: center; flex-shrink: 0;
    }
    .notif-icon-wrap.payment { background: rgba(44,182,125,0.2); color: var(--accent-teal); }
    .notif-icon-wrap.recharge { background: rgba(127,90,240,0.2); color: var(--accent-purple); }
    .notif-icon-wrap.sms { background: rgba(255,137,6,0.2); color: var(--accent-amber); }
    .notif-icon-wrap.email { background: rgba(59,130,246,0.2); color: #3b82f6; }
    .notif-icon-wrap.default { background: rgba(255,137,6,0.2); color: var(--accent-amber); }
    .notif-content { flex: 1; }
    .notif-title-row { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
    .notif-content strong { font-size: 15px; }
    .notif-type-badge {
      font-size: 10px; font-weight: 700; padding: 2px 8px; border-radius: 10px; text-transform: uppercase;
    }
    .notif-type-badge.in_app { background: rgba(127,90,240,0.2); color: var(--accent-purple); }
    .notif-type-badge.sms { background: rgba(255,137,6,0.2); color: var(--accent-amber); }
    .notif-type-badge.email { background: rgba(59,130,246,0.2); color: #3b82f6; }
    .notif-content p { color: var(--text-secondary); font-size: 13px; margin: 4px 0; }
    .notif-time { font-size: 12px; color: var(--text-secondary); }
    .unread-dot {
      width: 10px; height: 10px; border-radius: 50%;
      background: var(--accent-purple); flex-shrink: 0; margin-top: 6px;
    }
    ...
    this.notificationService.markAllAsRead()
      ...
      .subscribe({
        next: () => {
          ...
          this.cdr.markForCheck();
          this.snackBar.open('All notifications marked as read', 'Close', { duration: 2000, panelClass: ['success-snackbar'] });
        }
      });
  }
